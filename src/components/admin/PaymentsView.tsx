...
import { PayRun, PaymentBatch, PaymentItem } from "@/types";
...
import {
  ...
  Send, Loader2, ArrowLeft, XCircle, RefreshCw
} from "lucide-react";
...
const PaymentsView = () => {
  ...
  const createBatch = async (run: PayRun) => {
    ...
    try {
      ...
      const eligibleItems = run.lineItems.filter(li => {
      ...
      const { data: batch, error } = await supabase
        ...
        .insert({
          ...
          payment_count: eligibleItems.length,
          initiated_by: userId,
        } as any)
        .select()
        .single();
      ...
      const paymentItems = eligibleItems.map(li => {
        const emp = employees.find(e => e.id === li.employeeId);
        return {
      ...
      const missingBank = run.lineItems.length - eligibleItems.length;

      toast({
        title: "Payment batch created",
        description: `${eligibleItems.length} payments queued for processing.${missingBank > 0 ? ` ${missingBank} employee(s) skipped — no bank info on file.` : ""}`,
      });
  ...
  const exportBatchCSV = (batch: PaymentBatch) => {
    const header = "Employee,Amount,Bank,Account (last 4),Status\n";
    const rows = batch.items.map(i =>
      `${i.employeeName},${i.amount.toFixed(2)},${i.bankName || "N/A"},${i.accountLastFour || "N/A"},${i.status}`
    ).join("\n");
    ...
    <div className="space-y-4 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Create payment batches from approved pay runs and track direct deposit status.
          </p>
        </div>
        <button
      ...
      {approvedRuns.length > 0 && (
        <div className="glass-card rounded-xl p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Create a payment batch to initiate direct deposits.</p>
              ...
              <div key={run.id} className="flex items-center justify-between border-b border-border last:border-0 pb-3 last:pb-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {missingBank > 0 && (
                      <span className="text-warning ml-2">⚠ {missingBank} without bank info</span>
                    )}
                  </p>
                </div>
                {hasBatch ? (
